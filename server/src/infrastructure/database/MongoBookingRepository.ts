...
const slotSchema = new Schema<Slot>({
  ...
  slotType: { type: String, enum: ['HD-morning', 'HD-evening', 'Fullday'], required: true },
  slotPrice:{type:Number},
  status: { type: String, enum:['available','booked','cancelled','notavailable'],default:'available'},
  hallId: {
  ...
  export class MongoBookingRepository implements BookingRepository {
    ...
    async createBooking(booking: Booking, slot: Slot): Promise<Booking> {
      ...
      try {
        ...
        if (slotCreated) {
          const newBooking = new BookingModel({...booking,selectedSlot:slotCreated._id});
    ...
    async update(id: string, bookingData: Partial<Booking>): Promise<Booking | null> {
      ...
      return BookingModel.findByIdAndUpdate(id, bookingData, { new: true }).exec();
    }
    ...
    async updateSlot(id: string, slotData: Partial<Slot>): Promise<Slot | null> {
     ...
     return this.updateSlotAndBooking(id)
     else
    return SlotModel.findByIdAndUpdate(id, slotData, { new: true }).exec();
    }
    ...
    async getAvailableSlots(startDate: Date, endDate: Date,hallid:string): Promise<Slot[]> {
